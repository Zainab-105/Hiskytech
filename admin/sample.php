...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            foreach ($_POST['features'] as $index => $feature) {
                ...
                if (isset($_FILES['feature_images']['name'][$index]) && $_FILES['feature_images']['name'][$index] !== '') {
                    ...
                    if (move_uploaded_file($_FILES['feature_images']['tmp_name'][$index], $image_path)) {
                        ...
                        throw new Exception("Error moving uploaded file for feature ID: $feature_id.");
                    }
                }
            }
    ...
    if (isset($_FILES['technology_images'])) {
        foreach ($_FILES['technology_images']['name'] as $index => $image_name) {
            if ($image_name) {
                $image_path = 'uploads/' . basename($image_name);
                move_uploaded_file($_FILES['technology_images']['tmp_name'][$index], $image_path);

                $update_image_sql = "UPDATE technology_images SET image_path = ? WHERE project_id = ? LIMIT 1";
                $stmt_image_update = $conn->prepare($update_image_sql);
                $stmt_image_update->bind_param('si', $image_path, $project_id);
...
<script>
    // JavaScript functions to add more input fields for problem statements, solutions, etc.
    function addProblemStatement() {
        const container = document.getElementById('problem-statements');
        const input = document.createElement('input');
        input.type = 'text';
        input.name = 'problem_statements[]';
    ...
    function addFeature() {
        ...
        textarea.classList.add('form-control', 'editor');
        textarea.placeholder = 'Feature Description';
        featureDiv.appendChild(textarea);
        const fileInput = document.createElement('input');
        fileInput.type = 'file';
        fileInput.name = 'feature_images[]';
        fileInput.classList.add('form-control', 'mt-2');
        featureDiv.appendChild(fileInput);
        container.appendChild(featureDiv);
    }

    function addTechImage() {
        const container = document.getElementById('technology-images');
        const fileInput = document.createElement('input');
        fileInput.type = 'file';
        fileInput.name = 'technology_images[]';
        fileInput.classList.add('form-control', 'mt-2');
